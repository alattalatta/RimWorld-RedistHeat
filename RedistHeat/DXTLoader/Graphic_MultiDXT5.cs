...
namespace RedistHeat
{
	public class Graphic_MultiDXT5 : Graphic
	{
		...
		public override void Init(GraphicRequest req)
		{
			...
			array[0] = DXTLoader.LoadTextureDXT("LT_RedistHeat", req.path + "_back", TextureFormat.DXT5);
			if (array[0] == null)
			{
				Log.Error("Failed to find any texture while constructing " + ToString());
				return;
			}
			array[1] = DXTLoader.LoadTextureDXT("LT_RedistHeat", req.path + "_side", TextureFormat.DXT5);
			if (array[1] == null)
			{
				array[1] = array[0];
			}
			array[2] = DXTLoader.LoadTextureDXT("LT_RedistHeat", req.path + "_front", TextureFormat.DXT5);
			if (array[2] == null)
			{
				array[2] = array[0];
			}
