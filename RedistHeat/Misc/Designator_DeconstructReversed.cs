...
namespace RedistHeat
{
	public class Designator_DeconstructReversed : Designator
	{
		...
		{
			return (from t in Find.ThingGrid.ThingsAt( loc ) orderby t.def.altitudeLayer ascending select t).FirstOrDefault( current => CanDesignateThing( current ).Accepted );
		}
		...
		public override void DesignateThing(Thing t)
		{
			...
			if (Game.GodMode || Mathf.Approximately(innerIfMinified.GetStatValue(StatDefOf.WorkToMake), 0 ) || t.def.IsFrame)
			{
